...
| Decision                                                                 | Why                                                                                                     |
| ------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| Final band = family A (`medianCorrectionCommitsAfterOpen`) when present; family B (`mergedWithoutHumanEditRatio`) feeds `agreement` only and never raises the band A decided | Declarative/optimistic-signal principle already used elsewhere in the grid: a corroborating signal can lower confidence, never lift a level |
| When family A is absent, family B decides alone (at `sufficiency = 0.7`) instead of `err`ing | The issue's confidence section defines a `sufficiency = 0.7` case for "only one family present" — that case is unreachable if a missing family A always erred; a single-family reading is what the spec's own confidence formula anticipates |
