...
- **Regression** — the four sample profiles (`perceval` Red, `bohort` Blue, `leodagan` Green, `arthur` Copper): no wired axis may read *below* the assigned level. They are a guardrail, not a tuning target — thresholds are authored from the AIDD reference grid, then checked here. Axes already calibrated against the four get the stronger check (exact level match); listed in `EXACT_AXES` in `packages/core/test/regression/known-profiles.test.ts`. Calibrated so far: `size`.
