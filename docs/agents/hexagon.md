...
applies_to: "packages/core/src/**,packages/core/presets/**"
read_when: reviewing a change under packages/core/src/ or packages/core/presets/, or deciding where new code belongs
---
...
Nothing under `packages/core/src/core/**` imports from `packages/core/src/adapters/`, `packages/core/src/criteria/`, `packages/core/src/cli/`, or a runtime
...
that still couples the core to an adapter's shape). Reference: `packages/core/src/core/index.ts` is the core's
...
to JSON is the outbound adapter's job. Reference: `packages/core/src/adapters/outbound/json-evaluation.ts`.
...
a validated model. Reference: `packages/core/src/adapters/inbound/json-profile.ts`, `packages/core/src/adapters/inbound/json-grid.ts`.
...
they live in `packages/core/presets/*.json` under a bundle entry's `params` and reach a criterion through
...
Reference: `packages/core/src/criteria/size/pr-feature-size.ts` (`PARAM_DEFAULTS` merged with `context.params`).

## The core names no grid, axis or "AIDD" {#no-domain-vocab-in-core}

Code under `packages/core/src/core/**` hardcodes no axis id, no level id, and not the word "AIDD". The AIDD grid
is one preset among others; the core reads axes and levels from whatever `Grid` it is handed.
Reference: the header comment in `packages/core/src/core/model/profile.ts`.
