...
interface Props {
  ...
  /** A tighter variant for the rail / history rows. */
 ...
 * (`scale` empty).
 */
export function Ladder({ scale, current, compact = false }: Props): ReactNode {
  if (scale.length === 0) return null;
  const at = current === null ? -1 : scale.indexOf(current);

  return (
    <ol className={`ladder${compact ? ' compact' : ''}`} aria-label="level scale">
      {scale.map((label, i) => {
        const state = at < 0 ? 'none' : i < at ? 'below' : i === at ? 'here' : 'above';
        return (
          <li key={label} className={`ladder-step ${state}`} aria-current={i === at ? 'step' : undefined}>
